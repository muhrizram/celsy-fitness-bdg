...
@section('content')
    <div class="basis-11/12 bg-blue-500 grow p-5">
        ...
        <form action="{{ route('makanan.update', $foods->id) }}" method="post">
            @method('put')
            @csrf
            <div class="mb-6">
                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                    Makanan</label>
                <input type="text" id="name" name="name" value="{{ old('name', $foods->name) }}"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5    "
                    required>
            </div>
            <div class="mb-6">
                <label for="calory" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Kalori per
                    100 gram</label>
            ...
            <div class="mb-6">
                <label for="protein" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jumlah Protein
                    per 100 gram</label>
