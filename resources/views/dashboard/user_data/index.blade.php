...
@section('content')
    <div id="content">
        <h1 class="font-bold text-2xl md:pl-64 p-4 text-center">Pengguna Aplikasi</h1>
        <div class="md:pl-72 p-4">
            {{-- Header --}}
            <div class="flex flex-wrap md:flex-nowrap mb-5 justify-end items-center w-full">

                {{-- Section Search --}}
                <div class="w-full flex justify-end relative md:w-fit mt-10 md:mt-0">
                    <form class="w-full flex justify-end" action="/manager/pengguna-aplikasi">
                        <button type="submit"
            ...
            <div class="bg-white hidden lg:block">
                <div class="grid grid-cols-4 bg-slate-900 h-14 items-center border-b-4 border-yellow-300">
                    <div class="font-semibold p-3">Nama</div>
                    <div class="font-semibold">Usia</div>
                    <div class="font-semibold">Status</div>
                    <div class="font-semibold">Aksi</div>
                </div>
                @forelse($userdata as $number => $user_data)
                    <div
                        class="grid grid-cols-4 items-center h-fit {{ ($number + 1) % 2 == 0 ? 'bg-gray-600' : 'bg-gray-500' }}">
                        <div class="p-3 font-semibold">{{ $user_data->pengguna->name }}</div>
                        <div class=""><span
                        ...
                        <div class="my-2">
                            <button onclick="location.href='/manager/pengguna-aplikasi/{{ $user_data->pengguna->username }}';"
                                class="bg-red-500 p-3 rounded-lg hover:bg-red-600">Lihat</button>
                        </div>
                    </div>
                @empty
                    <div class="bg-gray-600 font-bold text-center p-3">
                        Data Pengguna Aplikasi tidak Tersedia.
                    </div>
                @endforelse
            </div>
            {{-- END Content for PC --}}
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:hidden">
                @forelse($userdata as $user_data)
                    <a href="/manager/pengguna-aplikasi/{{ $user_data->pengguna->username }}">
                        <div
                            ...
                            <div class="flex flex-col w-full">
                                <h1 class="border-b border-slate-800 group-hover:border-white">
                                    {{ $user_data->pengguna->name }}</h1>
                                <h1 class="border-b border-slate-800 group-hover:border-white">
                                    {{ \Carbon\Carbon::parse($user_data->date_of_birth)->age }}</h1>
                                <h1 class="border-b border-slate-800 group-hover:border-white">
                                    @if ($user_data->pengguna->level == 2)
                                        Admin
                                    @elseif($user_data->pengguna->level == 1)
            ...
            <div class="my-5 w-fit mx-auto">
                {{ $userdata->links('pagination::tailwind') }}
            </div>
            {{-- END Pagination --}}
